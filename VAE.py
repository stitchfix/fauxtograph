...
class ImageAutoEncoder():
	...
	def __init__(self, picture_width = 75, picture_height = 100, color_channels=3, encode_layers = [1000,600,300], 
		decode_layers = [300, 800, 1000], latent_width = 100 , GPU_flag = -1):
	...
	def load(self, filepath, img_width=75, img_height=100, color_channels=3):
		if not os.path.exists(filepath):
			...
			layer_arr = np.array([self.model.parameters[i].shape[0] for i in xrange(1,len(self.model.parameters),2)])



			self.encode_sizes = layer_arr[np.where(layer_arr == 22500)[0][0]+1:-1].tolist()
			self.decode_sizes = layer_arr[:np.where(layer_arr == 22500)[0][0]].tolist()
